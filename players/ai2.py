...
class Node:
    ...
    def __lt__(self, other):
        ...
        if self.player == 1 and other.player == 1:
            return self.score < other.score
        
        if self.player == 2 and other.player == 2:
            return self.score > other.score
...
class AIPlayer:
    ...
    def get_move(self, state: np.array) -> Tuple[int, int]:
        ...
        best = float("inf") if self.player_number == 1 else -float("inf")
        
        while leaf :
            top:Node = heappop(leaf)
            if top.move_number - self.root.move_number < 2 and abs(top.static_score) != float("inf"):
                ...
                # prune = int(len(top.children))
                # children = sorted(top.children)
                # print([c.score for c in children])
                # children = children[:prune]
